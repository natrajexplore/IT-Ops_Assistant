...
# Per-key fixed-window counter, in memory. Fine for one process; across
# multiple uvicorn workers each worker would count independently, so a real
# multi-worker deployment needs a shared store (Redis) instead - noted, not built.
...
def enforce_rate_limit(api_key: str = Depends(require_api_key)) -> None:
    ...
    if len(timestamps) >= RATE_LIMIT:
        retry_after = round(RATE_WINDOW - (now - timestamps[0]), 1)
        raise HTTPException(
            status_code=429,
            detail=f"Rate limit exceeded: {RATE_LIMIT} requests per {RATE_WINDOW:.0f}s",
...
async def chat(req: ChatRequest) -> ChatResponse:
    # Generated up front (not just when a pause happens) so it can serve as
    # the group_id that links this run to whatever /approve resumes it -
    # both show up as one grouped thread at platform.openai.com/traces.
    request_id = uuid.uuid4().hex
    with trace(workflow_name="IT Assistant Chat", group_id=request_id) as t:
        print(f"[trace] chat request_id={request_id} trace_id={t.trace_id}")
...
async def approve(req: ApproveRequest) -> ChatResponse:
    ...
    with trace(workflow_name="IT Assistant Chat", group_id=req.request_id) as t:
        print(f"[trace] approve request_id={req.request_id} trace_id={t.trace_id}")
